...
Data access patterns and Entity Framework Core conventions for the Medley modular monolith.
...
5. **Vector search** for AI/ML similarity queries (requires Azure SQL/SQL Server with vector support)
   ```csharp
   using Microsoft.EntityFrameworkCore.SqlTypes;

   // Entity property using SqlVector<T> type
   ...
   // Query with vector distance (distance metric as first parameter)
   var searchVector = new SqlVector<float>(queryEmbedding);
   var similar = await context.Products
       .OrderBy(p => EF.Functions.VectorDistance("cosine", p.Embedding, searchVector))
       .Take(10)
       .ToListAsync();
   ```
...
- [Database Migrations](../database-migrations.md) - Per-module migration patterns
- [CQRS Patterns](cqrs-patterns.md) - Command/Query separation
- [SharedKernel](shared-kernel.md) - Repository and UnitOfWork abstractions
